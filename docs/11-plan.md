...
| v1.24 | 2026-08-20 | FE-1~FE-9 완료 이후 UI 개선(정식 Task 외 후속 작업, DB/BE/FE Task 체크박스 변경 없음): (1) `frontend/.env` 부재로 `VITE_API_BASE_URL`이 `undefined`가 되어 로그인/회원가입이 전부 실패하던 버그 수정, (2) 전 화면에 뒤로가기(고정 목적지)/로그아웃 버튼(`TopNav`) 및 공통 상단 브랜드 바(`AppHeader`) 신설, (3) 로그인/회원가입 화면가운데 정렬 카드 레이아웃 및 프레시밀 실제 브랜드 로고/컬러(`#416922`)/`Poppins` 폰트 적용(스타벅스 참고 색상 전면 교체), (4) 응모 확정 시 취소 불가 확인창(`window.confirm`) 추가, (5) 이벤트 상세를 4개 섹션 카드로 재구성, (6) 보유 포인트 배지/이벤트 카드 제목·기간/상태 배지(`예정`)의 시각적 대비 개선. 테스트 110개 통과, `npm run build` 통과. `docs/6-project-principle.md`(v1.4)·`docs/8-wireframe.md`(v1.4)·`docs/12-style.md`(v1.3)에 위 변경사항 반영 완료 |
